...
| **P3** | Source code includes copyright headers, license, software bill of materials | Yes | **Mitigated** | Copyright header present in `main.py:1`, `Dockerfile:1`, `requirements.txt:1`, `client.py:1`. `LICENSE.txt` exists at project root. |
...
| **R2** | Documentation enumerates all resources created | No | **Mitigated** | `README.md` documents all resources: AgentCore runtime, ECR repository, IAM task role, CloudWatch log group. Architecture diagrams in `diagrams/` and `architecture-diagram.yaml` provide visual inventory. |
...
| **ECR3** | Container runs as non-root user | Yes | **Mitigated** | `Dockerfile:22-24`: `useradd -m -u 1000 bedrock_agentcore` + `USER bedrock_agentcore`. Container runs as UID 1000. |
...
| ID | Question | Release Blocker | Status | Evidence |
|----|----------|----------------|--------|----------|
| **IAM1** | IAM service roles use least-privilege | Yes | **Mitigated** | Task IAM role scoped to: `bedrock:InvokeModel` (specific models), `bedrock:ApplyGuardrail` (specific guardrail), `bedrock-agentcore:*` (own runtime), `ecr:GetAuthorizationToken`, `logs:*` (own log group). |
| **IAM2** | Custom IAM policies use least privileges | Yes | **Mitigated** | No custom IAM policies with `*` actions for Bedrock. Permissions scoped to specific service actions and resource ARNs. |
...
| **IAM5** | No long-term access keys used | Yes | **Mitigated** | No access keys generated or stored. `Dockerfile:12-14` explicitly documents runtime credential injection via IAM task roles (temporary STS credentials). `.gitignore` blocks `.env` files from being committed. |
